...
DESCRIPTION:
Function can identify an invalid input.
"
...
. "$___target"
# NOTICE: use process code because MacOS can produce the entire function
#         instead yielding false positive due to matches with its internal
#         'not found' checker.
if [ $? -ne 0 ]; then
